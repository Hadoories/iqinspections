  ...
  return (
    <header className="sticky top-0 z-40 shadow-sm shadow-sky-200/60 backdrop-blur">
      <div className="bg-gradient-to-r from-sky-800 to-sky-600 text-white">
        <div className="mx-auto flex max-w-6xl flex-col items-center gap-0.5 px-2.5 py-0.5 text-center text-[10px] font-semibold sm:flex-row sm:items-center sm:justify-between sm:gap-2 sm:px-4 sm:py-2 sm:text-left sm:text-xs md:px-6">
          ...
          <div className="flex flex-wrap items-center justify-center gap-1.5 text-white/90 sm:justify-end sm:text-xs">
            <a
              href={siteConfig.contact.phoneHref}
              className="inline-flex items-center gap-1.5 rounded border border-white/30 px-2.5 py-0.5 transition hover:bg-white/10"
            >
              ...
              <ul className="space-y-0.5 text-sm text-slate-600">
                {serviceLinks.map((service) => (
                  <li key={service.href}>
                    <Link
                      href={service.href}
                      className={`relative block rounded px-3 py-2 font-medium transition-all duration-200 hover:-translate-y-0.5 hover:bg-sky-50 hover:text-slate-900 hover:shadow-sm ${
      ...
      {mobileOpen && (
        <div className="border-t border-sky-100 bg-white lg:hidden">
          <div className="space-y-2 px-2 py-2 md:px-6">
            <div className="flex flex-col gap-1 text-[12px] font-semibold text-slate-600">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`rounded px-1.5 py-1 transition hover:bg-slate-50 ${
            ...
            <div className="space-y-1">
              <button
                ...
                className="flex w-full items-center justify-between rounded border border-sky-100 px-1.5 py-1 text-left text-[12px] font-semibold text-slate-600 transition hover:border-sky-200"
              >
                <span>Services</span>
                <ChevronDownIcon
                  className={`h-4 w-4 text-slate-400 transition-transform ${mobileServicesOpen ? "rotate-180" : ""}`}
                />
              </button>
              <div
                className={`mt-2 overflow-hidden rounded-xl border border-sky-50 transition-all duration-300 ${
                ...
                <div className="space-y-0.5 px-1.5 py-1">
                  {serviceLinks.map((service) => (
                    <Link
                      ...
                      className={`block rounded px-1.5 py-1 text-[12px] font-semibold text-slate-600 transition-all duration-200 hover:translate-x-1 hover:bg-sky-50 hover:text-slate-900 ${
                        pathname.startsWith(service.href)
                          ? "bg-sky-50 text-slate-900"
                          : ""
                      }`}
            ...
            <Link
              ...
              onClick={() => setMobileOpen(false)}
              className="btn btn-primary w-full justify-center py-1.5 text-[12px]"
            >
              {siteConfig.nav.cta.label}
            </Link>
          </div>
